...
# load data
DIR = '/home/ml/Personal/Code/MeachineLearning/Online-Realtime-Action-Recognition-based-on-OpenPose/Action/training/'
raw_data = pd.read_csv(DIR + 'data_scene.csv', header=0)
dataset = raw_data.values
# X = dataset[:, 0:36].astype(float)
# Y = dataset[:, 36]
# X = dataset[0:3289, 0:36].astype(float)  # 忽略run数据
# Y = dataset[0:3289, 36]  # 与训练的数据data_scene.csv行数一直
X = dataset[:, 0:36].astype(float)  # 忽略run数据
Y = dataset[:, 36]
...
# encoder_Y = [0]*35 + [1]*232 + [2]*417 + [3]*87  # 要根据训练数据进行改变
